...
## Generating Large Image Sets for Load Testing

This repository includes a helper script that takes **one source image** and produces **many transformed variants** until a target directory size is reached. This is useful for stress-testing galleries, CDNs, network performance, or caching layers.

### Usage

```
python tools/generate_images.py --input <source_image> --out web/images --target-gb 2.0 --format jpg --quality 85
```
...
| `--out`       | Output directory (default: `web/images`)                     |
| `--target-gb` | Stop generating when output folder reaches this size (in GB) |
| `--format`    | Output format: `jpg`, `png` (default: `jpg`)                 |
| `--quality`   | Image quality for JPG/WebP (default: 85)                     |

### Example

```
python tools/generate_images.py --input images/my-photo.jpg --target-gb 2
```

This will generate geometric variants (rotations, flips, swaps, tiling, shifts, etc.) until the `web/images/` folder is ~2 GB.

Progress is printed during generation, e.g.:

```
Progress: 1.34 / 2.00 GB
```

### Performance

* The script is **multithreaded** (parallel file writes)
* Optimized transformations (NumPy + OpenCV rotate/flip)
* Can fill **2GB in a few seconds** on SSD
